...
// io/worker.cpp, which owns the bridge on its STA thread); task 02.04 landed
// io/window_hook.cpp (pulled in by io/app.cpp, spawned as the third thread).
// io/error.cpp holds the shared error vocabulary + diagnostic sink, so it
// precedes the adapters.
