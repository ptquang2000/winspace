<#
    ...
    $mod is SUPER (the Windows key); Win+<n> chords only register because NoWinKeys=1
    is baked into the winspace-e2e-nowinkeys snapshot (scripts/PROVISIONING.md), as
    for every other workspace-switch seam. Each seam stages its own desktop-count
...
BeforeAll {
    ...
    # Initial config: Win+1 / Win+2 switch, Win+Shift+R reloads. NO Win+3 bind yet —
...
"@
    ...
    # Clean edited config: Win+2 is GONE and Win+3 is NEW; the reload bind persists so
...
"@
    ...
    # also tries to add Win+3 — which must NOT take effect, since nothing is applied.
    $script:ConfigBroken = @"
`$mod = SUPER
bind = `$mod, 1, workspace, 1
bind = `$mod, 2, workspace, 2
...
Describe 'config-reload' {
    ...
    # Apply-new-bind. winspace boots on the initial config (no Win+3), the file is
    # rewritten to add Win+3 and drop Win+2, and `reload` fires. The Worker re-reads
    # + re-parses on its thread and hands the new Binds to the Hotkey thread, which
    # re-registers: Win+3 (newly added) now switches, and Win+2 (removed) no longer
    ...
    It 'reload: re-parses config and re-registers hotkeys live (new bind fires, removed bind does not)' -Tag 'config-reload' {
        ...
        try {
            ...
            # The newly-added Win+3 fires. Re-send inside the poll: the Hotkey thread
            ...
            Wait-Until -Because 'the newly-added Win+3 bind to switch to workspace 3' -Condition {
                Send-Chord 'Win+3'
                (Get-VdState).CurrentGuid -eq $guid3
            }
            (Get-VdState).CurrentGuid | Should -Be $guid3 -Because 'the reload re-registered the added bind'

            # The removed Win+2 no longer fires. Land on workspace 1, press Win+2, and
            ...
            (Get-VdState).CurrentGuid | Should -Be $guid1 `
                -Because 'the removed Win+2 bind was unregistered on reload, so it switches nothing'
        } catch {
    ...
    # live (the previously-working Win+2 still switches), the process never dies, and
    ...
    It 'reload: an invalid config keeps the last-good config live and logs diagnostics (no crash)' -Tag 'config-reload' {
        ...
        try {
            ...
            # Baseline: Win+2 works before the bad reload.
            Wait-Until -Because 'the initial Win+2 bind to switch to workspace 2' -Condition {
                Send-Chord 'Win+2'
            ...
            # The previously-working Win+2 STILL fires (last-good retained); the broken
            # file's attempted Win+3 does NOT (nothing was applied). Land on 1 first.
            ...
            Start-Sleep -Milliseconds 600
            (Get-VdState).CurrentGuid | Should -Be $guid1 `
                -Because 'the broken file was rejected whole, so its Win+3 bind was never registered'
            Wait-Until -Because 'the retained Win+2 bind to still switch to workspace 2' -Condition {
                Send-Chord 'Win+2'
                (Get-VdState).CurrentGuid -eq $guid2
            }
            ...
            # and its newly-added Win+3 fires.
            Set-WinspaceConfig -Content $script:ConfigAddWin3 | Out-Null
            Send-Chord 'Win+Shift+R'
            Wait-Until -Because 'the Worker to apply the good config after the bad one' -Condition {
                (Get-WinspaceLogText) -match 'reload: applied new config'
            }
            Wait-Until -Because 'the re-fixed Win+3 bind to switch to workspace 3' -Condition {
                Send-Chord 'Win+3'
                (Get-VdState).CurrentGuid -eq $guid3
            }
